...
typedef struct _hashmap_map{
	...
	PFdestroy destroy_fn;  /* optional data destructor */
} hashmap_map;

/*
 * Return an empty hashmap, or NULL on failure.
 */
map_t hashmap_new(size_t init_size, PFdestroy destroy_fn) {
...
/* Deallocate the hashmap and destroy all elements */
void hashmap_clear(map_t in)
{
	...
	for (i=0; i < map->table_size; i++)
	{
		...
		{
			if (map->destroy_fn)
				map->destroy_fn(map->data[i].data);
			else
